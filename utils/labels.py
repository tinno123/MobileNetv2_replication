...
if __name__ == '__main__':
    ...
    anchors_config = torch.tensor([
        [116, 90] ,[156, 198],[373, 236],
        [30, 61],[62, 45], [59, 119],
        [10, 13], [16, 30], [33, 23]
    ])

    result = build_labels([416, 416], targets, anchors_config)
    positive_mask = result[:, 5].bool()
    print(result[...,[6,7,8,9]][positive_mask])
    ...
    # out1 = torch.randn(16, 75, 13, 13)
    # out11 = out1.permute(0, 2, 3, 1)
    # out11 = out11.reshape(out11.shape[0], -1, 25)
    # print(out1[:,2 * 25:(2+1) * 25,9,9].shape)
    # print(out11[:,380,:].shape)
    # if (out1[:, 2 * 25:(2 + 1) * 25, 9, 9] == out11[:, 380, :]).all():
    #     print("相同")
