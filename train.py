...
from utils.train_tools import CosineDecayLR, seed_everything, GetLoader, GetOptimizer, load_by_shape_precise, \
...
def  train(
    ...
    for epoch in range(start_epoch,epoches):
        ...
        for step, (images,targets) in enumerate(train_loader):
            ...
            if fp16:
                with autocast("cuda"):
                    ...
                    output = model(images)
                    # 计算损失
                    total_loss ,acc= loss(output, targets)
        ...
        avg_acc  = avg_acc / len(val_loader)
        ...
        torch.save(checkpoint, Path(save_path) / "MobileNetV2.pth")
...
if __name__ == "__main__":
    ...
    # ----------------------------------------------#
    model_path = 'weights/MobelNetV2_Weight.pth'
    ...
    # ----------------------------------------------------#
    resume = False
    resume_model = "runs/train/20260421_225834/MobileNetV2.pth"

    # ----------------------------------------------------#
    #   获取存放数据集根目录
    # ----------------------------------------------------#
    # 数据集存放根目录
    root_dir = r"E:\hands\yolo"
    # 数据集路径
    ...
    #   获取模型
    # ----------------------------------------------------#
    model = MobileNetV2(num_classes=number_classes).to( device)
    ...
    #   获取损失函数
    # ----------------------------------------------------#
    loss = YoloLoss(number_classes).to(device)
    ...
    train(model,loss,optimizer,train_loader,val_loader,device,fp16,epoches,start_epoch,freeze_epoch,lr_decay,save_path=save_path)
